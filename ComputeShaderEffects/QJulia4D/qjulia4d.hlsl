...
{
	uint4 uc = round(clamp(ui, 0, 1) * 255);
              ...
              bool renderShadows)           // flag for turning self-shadowing on/off
{
   ...
   const float4 backgroundColor = float4( 0, 0, 0, 0 );  //define the background color of the image
...
//****************************************************************************
{ 
	if (DTid.x > (uint)(rectWidth - 1))
	{
		return;
	}

    float2 coord	= float2((float)DTid.x + offsetX, (float)DTid.y + offsetY);
    float2 size     = float2((float)c_width, (float)c_height);
    float scale     = min(size.x, size.y);
    float2 half     = float2(0.5f, 0.5f);
	float2 position = (coord - half * size) / scale * BOUNDING_RADIUS_2 * zoom;
    ...
    output[DTid.y * rectWidth + DTid.x] = Float4ToUint(color);
}
